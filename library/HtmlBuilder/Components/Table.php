...
{
    public $fixedTop       = true;
    public $fixedLeft      = true;
    public $fixedRight     = true;
    public $height         = null; // 最大高度如果定义，那么则固定头部 fixedTop = 1
    public $canEdit        = "false";   // 是否可以编辑，1 表示永远可用，字符串表示数据行的字段为真，比如 canEdit = 'canEdit' ,那么数据行存在 canEdit 字段，并且字段为true时则显示编辑，否则不显示
    public $canDelete      = false;   // 是否可以删除，1 表示永远可用，字符串表示数据行的字段为真
    public $selectMode     = 'null'; // 选择模式, null:不可选择，single:单选，multi:多选
    ...
    public $updateApi      = ''; // 编辑数据的API
    public $deleteApi      = ''; // 删除数据的API
